...
def test_dashboard_session_creation(client):
...
def test_checkout_session_creation(client):
...
def test_payout_configuration(client):
...
def test_tax_form_generation(app_context):
